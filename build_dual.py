#!/usr/bin/env python3
"""Dual-strain split tag — a FULL colour border per half.
...
The app calls build_sheet(); the __main__ block below only writes demo sheets.
...
STRAIN_TOP_GAP = 10.0 # space left under the brand line above the strain
...
def build(src, out, pairs=None, quiet=False):
    ...
    for (x0, y0, x1, y1) in tags:
        ...
        for hx0, hx1 in [(x0, xm - GAP / 2), (xm + GAP / 2, x1)]:
            ...
            # Sit the strain just under the brand line rather than centring it,
            # so the slack collects BELOW it — that is where the deal badge goes.
            s_hi = iy1 - 27 - STRAIN_TOP_GAP
            strain_r = (ix0, s_hi - STRAIN_MAX_H, ix1, s_hi)
...
if __name__ == "__main__":
    ...
    if size.startswith("3"):
        build("Sativa_Prerolls.pdf", "Dual_Split_35in_DEMO.pdf")
    else:
        build("Sativa_Prerolls_4in.pdf", "Dual_Split_4in_DEMO.pdf")
...
def build_sheet(rows, tmpdir, size="4"):
    ...
    writer.write(buf)
    buf.seek(0)
    return buf.getvalue()BORDER_W = 5.0        # thickness of each half's colour frame (pt)
GAP = 2.5             # white gap between the two halves (tight)
INSET = 1.5           # pull the frames just inside the tag cell
MASK = 12.0           # how far in to paint out the original border ring
STRAIN_MAX_H = 30.0   # strain box height = the size ceiling (matches "LEELA HERB")
